...
/* CUSTOM:
 * -- replace transform with m_Handle
 * -- replace ((RectTransform)m_Handle).anchorPosition --> localPosition
 * -- add m_Background, m_Handle
 * -- BeginInteraction: add case Behaviour.ExactPositionWithDynamicOrigin: m_Background.localPosition = pointerDown;
 * -- EndInteraction: add m_Background.localPosition = m_StartPos;
 * -- add m_Background, m_Handle to editor; assign value; show in inspector
 * -- add:
...
namespace EnhancedOnCreenStick {
    ...
    public class EnhancedOnScreenStick : OnScreenControl, IPointerDownHandler, IPointerUpHandler, IDragHandler {
        ...
        private void Start() {
            ...
            if (!(m_Handle is RectTransform))
                return;

            m_StartPos = ((RectTransform)m_Handle).localPosition;
            ...
            dynamicOrigin.transform.SetParent(m_Handle);
        ...
        private void BeginInteraction(Vector2 pointerPosition, Camera uiCamera) {
            var canvasRectTransform = UGUIOnScreenControlUtils.GetCanvasRectTransform(m_Handle);
            ...
            switch (m_Behaviour) {
                ...
                case Behaviour.ExactPositionWithDynamicOrigin:
                    ...
                    m_PointerDownPos = ((RectTransform)m_Handle).localPosition = pointerDown;
                    m_Background.localPosition = pointerDown;
                    break;
            }
        }
        ...
        private void MoveStick(Vector2 pointerPosition, Camera uiCamera) {
            ...
            switch (m_Behaviour) {
                case Behaviour.RelativePositionWithStaticOrigin:
                    ...
                    ((RectTransform)m_Handle).localPosition = (Vector2)m_StartPos + delta;
                    break;
                ...
                case Behaviour.ExactPositionWithDynamicOrigin:
                    delta = Vector2.ClampMagnitude(delta, movementRange);
                    ((RectTransform)m_Handle).localPosition = m_PointerDownPos + delta;
                    break;
            }
        ...
        private void EndInteraction() {
            ((RectTransform)m_Handle).localPosition = m_PointerDownPos = m_StartPos;
            if (m_Behaviour == Behaviour.ExactPositionWithDynamicOrigin)
                m_Background.localPosition = m_StartPos;
        ...
        private void OnDrawGizmosSelected() {
            ...
            var parentRectTransform = m_Handle.parent as RectTransform;
        ...
        private void UpdateDynamicOriginClickableArea() {
            var dynamicOriginTransform = m_Handle.Find(kDynamicOriginClickable);
        ...
        internal class EnhancedOnScreenStickEditor : UnityEditor.Editor {
            ...
            public void OnEnable() {
                ...
                m_UseIsolatedInputActions = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_UseIsolatedInputActions));

                m_Behaviour = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_Behaviour));
                m_ControlPathInternal = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_ControlPath));
                m_MovementRange = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_MovementRange));
                m_DynamicOriginRange = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_DynamicOriginRange));
                m_PointerDownAction = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_PointerDownAction));
                m_PointerMoveAction = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_PointerMoveAction));
                m_Background = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_Background));
                m_Handle = serializedObject.FindProperty(nameof(EnhancedOnScreenStick.m_Handle));
            }
            ...
            public override void OnInspectorGUI() {
                ...
                m_ShowDynamicOriginOptions.target = ((EnhancedOnScreenStick)target).behaviour ==
                ...
                if (EditorGUILayout.BeginFadeGroup(m_ShowDynamicOriginOptions.faded)) {
                    ...
                    if (EditorGUI.EndChangeCheck()) {
                        ((EnhancedOnScreenStick)target).UpdateDynamicOriginClickableArea();
                    }
                    EditorGUI.indentLevel--;
                }
